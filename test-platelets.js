...
async function testPlateletGeneration() {
  ...
  console.log(
    `📊 Plate radius: ${plate.radius} radians (${(
      (plate.radius * 180) /
      Math.PI
    ).toFixed(2)}°)`
  );
  console.log(
    `📊 Plate position: (${plate.position.x.toFixed(
      2
    )}, ${plate.position.y.toFixed(2)}, ${plate.position.z.toFixed(2)})`
  );

  if (platelets.length === 0) {
    console.log(
      "❌ No platelets generated - there might be an issue with the generation logic"
    );
  } else if (platelets.length === 1) {
    console.log(
      "⚠️ Only 1 platelet generated - this seems too few for a large plate"
    );
  } else {
    console.log(
      `✅ Generated ${platelets.length} platelets - this looks good!`
    );
  }
}
