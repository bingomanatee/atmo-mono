...
import { PLATE_TYPES, PlateIF } from '../types.atmo-plates';
...
describe('plateUtils', () => {
  ...
  describe('determineBehavioralType', () => {
    it('should determine continental-like plates correctly', () => {
      expect(determineBehavioralType(2.7, 2.8, 2.9)).toBe(PLATE_TYPES.CONTINENTAL);
    });
    ...
    it('should determine transitional plates correctly', () => {
      expect(determineBehavioralType(2.85, 2.8, 2.9)).toBe(PLATE_TYPES.TRANSITIONAL);
    });
